        ...
        this.setOnTouchListener(new View.OnTouchListener() {
            public boolean onTouch(View view, MotionEvent motionEvent) {
                ...
                if ((mode == Mode.DRAG && scale >= MIN_ZOOM) || mode == Mode.ZOOM) {
                    ...
                    //0, dx 0.0, max 0.0
                    // dx -0.0, max 0.0
                    //   if (maxDx == 0.0 && scale == 1.0 || maxDx == 0.0 && scale == 1.0) {

                    //TODO handling here (prevent drag to outside the screen)
                    dx = Math.min(Math.max(dx, -view.getWidth() / 3), view.getWidth() / 3);
                    dy = Math.min(Math.max(dy, -view.getHeight() / 3), view.getHeight() / 3);
                    //    }

                    // TODO ORGINAL CODE UNCOMMENT BELOW CODE
//
//                    dx = Math.min(Math.max(dx, -maxDx / 2), maxDx);
//                    dy = Math.min(Math.max(dy, -maxDy / 2), maxDy);
